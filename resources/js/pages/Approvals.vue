  ...
  <v-container style="max-width: 90%" class="mt-0 pt-0">
    ...
    <v-row class="mt-0 pt-0">
      ...
      <div class="v-col-6 text-center">
        <img src="/assets/images/grandiose-logo.jpg" style="max-height: 100px" />
        <br />
    ...
    <v-row>
      ...
      <div class="v-col-12 v-col-md-2 py-1">REMARKS<br />REASON FOR REQUEST</div>
    ...
    >
            ...
            <v-card class="bg-black">
              ...
              >
                <v-carousel-item reverse-transition="fade" transition="fade">
                  ...
                  >
                    <v-img :src="baseURL + '/file/' + item.attachment.path"></v-img>
                  </div>
                </v-carousel-item>
              </v-carousel>
            </v-card>
      ...
      <div v-if="is_commercial_manager" class="v-col-12 v-col-md-2 py-1 d-flex">
        <div>
      ...
      <div v-else-if="is_realeasing" class="v-col-12 v-col-md-2 py-1">
        <div>
          ...
          <div v-if="item.remarks_commercial">
            ...
            <pre>{{ item.remarks_commercial }}</pre>
            -------
          </div>
          <div v-if="item.remarks_release">
            <strong> Releasing</strong> <br />
            <pre>{{ item.remarks_release }}</pre>
            -------
          </div>
        </div>
        <v-textarea
      ...
      <div v-else class="v-col-12 v-col-md-2 py-1 d-flex justify-space-between">
        <div
        ...
        <div
          v-if="dataObj?.data?.status == 'complete' && checkLastApproval == 'receiver'"
        >
          <v-icon
            class="mt-3"
    ...
    <v-row class="mt-3" v-if="is_asset_supervisor">
      ...
      >
        ...
        PLEASE TICK CHECKBOX IF ASSET IS AVAILABLE BEFORE CLICKING THE APPROVE BUTTON
      </div>
    ...
    >
      ...
      >
        <div>
          ...
          AVAILABLE
          <v-icon :icon="mdiCloseThick" color="error" class="ml-5"></v-icon>
          NOT AVAILABLE
        </div>
        <div v-if="is_receiver" class="ml-5 d-flex">
          <div class="px-2 mr-5">|</div>
          <v-icon :icon="mdiCheckboxBlankOutline"></v-icon> TICK RECEIVED ASSETS
        </div>
        <div
          class="ml-6"
          v-if="dataObj.data?.status == 'complete' && checkLastApproval == 'receiver'"
        >
          ...
          RECEIVED
          <v-icon :icon="mdiTruckRemove" color="error" class="ml-5"></v-icon>
          NOT RECEIVE
        </div>
      </v-card>
    </v-row>
    ...
    <v-row v-if="dataObj.data?.request_approvals.length > 0">
      <div class="v-col-12 v-col-md-3 pa-0 mb-4">
        <div
          ...
          :style="`${
            requestStatus == 'cancelled' ? 'color: red;' : 'color: rgb(3, 167, 3);'
          } border: 1px solid #ccc; font-size: 17px;`"
        >
          {{ requestStatus == "cancelled" ? "CANCELLED" : "APPROVED" }}
        </div>
        <div class="text-center py-1 font-weight-bold">Requested By</div>
      ...
      >
        <div
          ...
          "
        >
          <div class="d-flex" v-if="item.status == 'awaiting-approval'">
            <div
              @click="rejectRequest(item)"
              class="cursor-pointer text-center py-5 v-col-6"
              :style="`border:1px solid #ccc; ${
                route.query.pid == item.profile_id && route.query.o == item.orders
          ...
          <div
            v-else-if="
              item.status == 'reject' || item.status == 'done' || item.status == 'pending'
            "
          >
            <div
        ...
        <div
          :class="`text-center py-1 font-weight-bold ${
            item.status == 'reject' ? 'text-error text-uppercase' : ''
          }`"
        >
          {{ item.status == "reject" ? "Rejected" : statusFn(item.approval_type) }}
          By
        </div>
        <div class="text-center py-1">
        ...
        <div class="v-col-12" v-if="item.reason_rejected" style="border: 1px solid red">
          Reason
  ...
  await axios
    ...
    .then((res) => {
      ...
      if (dataObj.value?.data?.request_approvals?.length > 0) {
        is_asset_supervisor.value = dataObj.value.data.request_approvals.filter(
          (o) =>
        ...
        is_commercial_manager.value = dataObj.value.data.request_approvals.filter(
          (o) =>
            o.profile_id == route.query.pid &&
            o.orders == route.query.o &&
            o.status == "awaiting-approval" &&
