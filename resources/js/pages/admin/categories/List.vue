<template>
    <v-container>
        <AppPageHeader title="Category List" />
        <v-row class="mb-3">
...
const searchData = () => {
    localStorage.setItem("category-search", encryptData(search.value));
    fetchAllData();
};
...
const saveData = async (data) => {
    ...
    await clientKey(authStore.token)
        .post("/api/categories/store-update/data", data)
        .then((res) => {
            sbOptions.value = {
                status: true,
                type: "success",
            ...
            freeForm.value = [
                { name: "title", label: "Category", required: true, type: "text" },
...
const nonPaginatedData = ref([]);
const fetchNonPaginated = async () => {
    await clientKey(authStore.token)
        .get(
            "/api/fetch/categories/non-paginated/data"
        )
        .then((res) => { 
            nonPaginatedData.value = res.data; 
            freeForm.value.push( { name: "category_id", label: "Parent Category", required: false, type: "select", data: nonPaginatedData.value });
        })
        .catch((err) => {
...
const editData = (data) => {
    ...
    dataObject.value.type = "category";
...
const deleteData = async (id, status) => {
    ...
    await clientKey(authStore.token)
        .post("/api/categories/status-change/data", form)
        .then((res) => {
            setTimeout(() => {
                sbOptions.value = {
                    status: true,
...
onMounted(() => {
    let vsearch = localStorage.getItem("category-search");
    if (vsearch) {
