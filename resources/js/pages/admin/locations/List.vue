<template>
    <v-container>
        <AppPageHeader title="Location List" />
        <v-row class="mb-3">
...
const searchData = () => {
    localStorage.setItem("location-search", encryptData(search.value));
    fetchAllData();
};
...
const saveData = async (data) => {
    ...
    await clientKey(authStore.token)
        .post("/api/locations/store-update/data", data)
        .then((res) => {
            sbOptions.value = {
                status: true,
                type: "success",
...
const freeForm = ref([
    { name: "title", label: "Location", required: true, type: "text" },
    { name: "code", label: "Code", required: true, type: "text" },
    { name: "area", label: "Area Location", required: false, type: "text" },
]);

const editData = (data) => {
    dataObject.value = Object.assign({}, data);
    dataObject.value.type = "location";
...
const deleteData = async (id, status) => {
    ...
    await clientKey(authStore.token)
        .post("/api/locations/status-change/data", form)
        .then((res) => {
            setTimeout(() => {
                sbOptions.value = {
                    status: true,
...
onMounted(() => {
    let vsearch = localStorage.getItem("location-search");
    if (vsearch) {
